...
for i in range(nSubj):
    ...
    n, bins, patches = plt.hist(walks, num_bins, normed=1, facecolor='orange', alpha=0.5)
    plt.xlabel('Node')
    plt.ylabel('Frequency')
    fig.savefig("".join(['/Users/stiso/Documents/Python/graphLearning/images/', str(i), 'mod_walk.png']))
    ...
    num_bins = 50
    # the histogram of the data
    n, bins, patches = plt.hist(walks, num_bins, normed=1, facecolor='blue', alpha=0.5)
    plt.xlabel('Node')
    plt.ylabel('Frequency')
    fig.savefig("".join(['/Users/stiso/Documents/Python/graphLearning/images/', str(i), '_ring_walk.png']))
    
    # write to csv
    csvfile = "ring_lattice.csv"
    with open(csvfile, "a") as fp:
        wr = csv.writer(fp, dialect='excel')
        wr.writerow(walks)
